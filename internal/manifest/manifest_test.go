...
func openManifest(t *testing.T) (*Manifest, string) {
	t.Helper()
...
func replayAll(t *testing.T, path string) []VersionEdit {
	t.Helper()
	m, err := Open(path)
	require.NoError(t, err)
	...
	require.NoError(t, m.Replay(func(e VersionEdit) {
...
func TestManifestSeqMonotonic(t *testing.T) {
	...
	for range 5 {
		require.NoError(t, m.Append(VersionEdit{
			AddedFiles: []FileEntry{{Path: "x.sst"}},
		}))
	}
	require.NoError(t, m.Close())
...
func TestManifestSeqResumesAfterReopen(t *testing.T) {
	...
	require.NoError(t, err)
	for range 3 {
		require.NoError(t, m1.Append(VersionEdit{AddedFiles: []FileEntry{{Path: "f.sst"}}}))
	}
	...
	m2, err := Open(path)
	require.NoError(t, err)
	require.NoError(t, m2.Replay(func(VersionEdit) {}))
	for range 2 {
		require.NoError(t, m2.Append(VersionEdit{AddedFiles: []FileEntry{{Path: "g.sst"}}}))
	}
...
func TestManifestPartialWriteTruncated(t *testing.T) {
	...
	m, err := Open(path)
	require.NoError(t, err)
	require.NoError(t, m.Append(VersionEdit{AddedFiles: []FileEntry{{Path: "a.sst"}}}))
	require.NoError(t, m.Append(VersionEdit{AddedFiles: []FileEntry{{Path: "b.sst"}}}))
	require.NoError(t, m.Close())
...
func TestManifestReplayRebuildLiveFiles(t *testing.T) {
	...
	require.NoError(t, m.Append(VersionEdit{
		AddedFiles:   []FileEntry{{Path: "c.sst"}},
		DeletedFiles: []FileEntry{{Path: "a.sst"}},
	}))
	require.NoError(t, m.Close())
